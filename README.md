...
| Repo | What it is |
|---|---|
| **[platform-framework](https://github.com/FilippDor/platform-framework)** | Canonical, versioned source of the shared infra modules (`config`, `schema`, `registry`, `validation`, `monitoring`, `feature_store`) + `scripts/bootstrap.sql` + the drift checker. Currently **v0.3.0**. Copied into each project at creation time — not a runtime dependency. |
...
- **All seven pillars exercised end-to-end** at MVP depth by the pilot.
  `CHURN_RISK` v1 is registered in `MODEL_REGISTRY.RETENTION`, scored rows
  land in `ML_DEV.CHURN_RISK.SCORES`, drift rows in
  `MONITORING.PUBLIC.DRIFT_LOG`.
- **Snowflake bootstrap** (`platform-framework/scripts/bootstrap.sql`) has
  been applied to the Dev account: databases `GOLDEN_LAYER`,
  `FEATURE_STORE`, `MODEL_REGISTRY`, `MONITORING`.
